...
deploy-v8:
	$(INSTALL) dvm/native-lib/*$(DYLIB) /usr/local/lib/
	$(LDCONFIG)
...
build:
	cd cmd/deb; go build $(LDFLAGS) -o ../../$(BINARY)-$(COMMIT)
	cd cmd/crashreporter; go build $(LDFLAGS) -o ../../deb-crashreporter
	-rm -f $(BINARY)
	ln -s $(BINARY)-$(COMMIT) $(BINARY)

build-linux:
	cd cmd/deb; GOOS=linux GOARCH=amd64 go build $(LDFLAGS) -o ../../$(BINARY)-linux
