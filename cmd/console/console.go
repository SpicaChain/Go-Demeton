...
import (
	...
	"github.com/cyber-demeton/go-demeton/cmd/console/library"
...
var (
	...
	nebJS       = library.MustAsset("deb-light.js")
)

// Deblet interface breaks cycle import dependency and hides unused services.
type Deblet interface {
...
// New a console by Config, deb.config params is need
func New(conf Config) *Console {
	...
	if conf.Deb != nil {
		c.jsBridge = newBirdge(conf.Deb.Config(), c.prompter, c.writer)
	} else {
...
func (c *Console) loadLibraryScripts() error {
	...
	if err := c.jsvm.Compile("deb-light.js", nebJS); err != nil {
		return fmt.Errorf("deb.js: %v", err)
	}
	return nil
}
...
func (c *Console) methodSwizzling() error {
	...
	if _, err := c.jsvm.Run("var Deb = require('deb');"); err != nil {
		return fmt.Errorf("deb require: %v", err)
	}
	if _, err := c.jsvm.Run("var deb = new Deb(bridge);"); err != nil {
		return fmt.Errorf("deb create: %v", err)
	}
	jsAlias := "var api = deb.api; var admin = deb.admin; "
...
func (c *Console) Setup() {
	...
	fmt.Fprint(c.writer, "Welcome to the Deb JavaScript console!\n")
}
