...
func accountList(ctx *cli.Context) error {
	...
	for index, addr := range deb.AccountManager().Accounts() {
...
func accountCreate(ctx *cli.Context) error {
	...
	addr, err := deb.AccountManager().NewAccount([]byte(passphrase))
...
func accountUpdate(ctx *cli.Context) error {
	...
	for _, address := range ctx.Args() {
		...
		err = deb.AccountManager().Update(addr, []byte(oldPassphrase), []byte(newPassword))
		if err != nil {
...
func accountImport(ctx *cli.Context) error {
	...
	addr, err := deb.AccountManager().Import([]byte(keyJSON), []byte(passphrase))
	if err != nil {
