...
var Wallet;
try {
    Wallet = require("../../deb.js");
} catch (e) {
    Wallet = require("demeton");
}
...
deb.setRequest(new HttpRequest(apiEndPoint));
...
function testTransfer(testInput, testExpect, done) {
    deb.api.getAccountState(from.getAddressString()).then(function (state) {
        ...
        if (!testInput.hasOwnProperty("payloadLength")) {
            tx = new Transaction(ChainID, from, toAddr, Unit.debToBasic(testInput.transferValue), parseInt(fromState.nonce) + testInput.nonceIncrement, testInput.gasPrice, testInput.gasLimit);
        } else {
            var payloadContent = new Array(testInput.payloadLength + 1).join("s");
            console.log("payloadcontent:" + payloadContent)
            tx = new Transaction(ChainID, from, toAddr, Unit.debToBasic(testInput.transferValue), parseInt(fromState.nonce) + testInput.nonceIncrement, testInput.gasPrice, testInput.gasLimit, payloadContent);
        }
                ...
                try {
                    if (true === testExpect.canSubmitTx) {
                        ...
                        deb.api.getAccountState(receipt.from).then(function (state) {
                            ...
                            return deb.api.getEventsByHash(resp.txhash);
...
function prepare(done) {
    ...
    deb.api.getAccountState(sourceAccount.getAddressString()).then(function (resp) {
        ...
        var tx = new Transaction(ChainID, sourceAccount, from, Unit.debToBasic(initFromBalance), parseInt(resp.nonce) + 1);
        tx.signTransaction();
...
describe('normal transaction', function () {
    ...
    it('[balanceOfFrom sufficient] (GasLimit * GasPrice + valueOfTx) > balanceOfFrom ' +
        ...
        // The TxBaseGasCount in deb is 20000
