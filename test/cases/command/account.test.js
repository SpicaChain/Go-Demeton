...
describe('deb account', () => {

    it('account new', (done) => {
        process.exec(deb + ' account new 123456', (err, stdout, stderr) => {
            try {
    ...
    it('account list', (done) => {
        process.exec(deb + ' account list', (err, stdout, stderr) => {
            try {
                expect(err).to.be.equal(null);
                expect(stderr).to.be.equal("");
    ...
    it('account update', (done) => {
        var child = process.exec(deb + ' account update ' + newAccount, (err, stdout, stderr) => {
            try {
                expect(err).to.be.equal(null);
                expect(stderr).to.be.equal("");
                console.log(stdout);
    ...
    it('account import', (done) => {
        var child = process.exec(deb + ' account import keydir/' + newAccount, (err, stdout, stderr) => {
            try {
                expect(err).to.be.equal(null);
                expect(stderr).to.be.equal("");
