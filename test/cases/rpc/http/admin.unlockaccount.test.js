...
var Wallet = require("../../../../cmd/console/deb.js/lib/wallet");
...
if (env === 'local') {
    deb.setRequest(new HttpRequest("http://localhost:8685"));//https://testnet.nebulas.io
	ChainID = 100;
...
function newAccount(testInput, done) {
    deb.admin.newAccount(testInput.passphrase).then(resp => {
...
function testUnlockAccount(testInput, testExpect, done) {
    ...
    deb.admin.unlockAccount(testInput.address, testInput.passphrase).then((resp) => {
