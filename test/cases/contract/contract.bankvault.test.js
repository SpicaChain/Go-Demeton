...
var deb = new Wallet.Deb();
deb.setRequest(new HttpRequest("http://localhost:8685"));
var ChainID = 100;
...
function checkTransaction(hash, callback) {
    ...
    deb.api.getTransactionReceipt(hash).then(function(resp) {
...
function deployContract(testInput, done) {
    ...
    deb.api.getAccountState(from.getAddressString()).then(function(state){
        fromState = state;
        ...
        tx.signTransaction();
        return deb.api.sendRawTransaction(tx.toProtoString());
    }).catch (function(err){
        console.log(err.error);
        done(err);
    }).then(function(resp){
        ...
        checkTransaction(resp.txhash, function(resp){
            ...
            deb.api.getAccountState(contractAddr).then(function(state){
...
function testSave(testInput, testExpect, done) {
    ...
    deb.api.getAccountState(contractAddr).then(function(state){
        ...
        return deb.api.getAccountState(from.getAddressString());
    }).then(function(state){
        ...
        deb.api.getAccountState(coinbase).then(function(state){
            console.log("[before save] coinbase state:" + JSON.stringify(state));
            coinBalanceBefore = state.balance;
        });
       
        return deb.api.estimateGas(from.getAddressString(), contractAddr,
        ...
        checkTransaction(resp.txhash, function(receipt){
            ...
            deb.api.getAccountState(receipt.from).then(function(state){
                ...
                return deb.api.getAccountState(receipt.to);
                ...
                return deb.api.getAccountState(coinbase);
            }).then(function(state){
...
function testTakeout(testInput, testExpect, done) {
    ...
    deb.api.getAccountState(contractAddr).then(function(state){
        ...
        deb.api.estimateGas(from.getAddressString(), contractAddr,
        ...
        checkTransaction(resp.txhash, function(receipt){
            try {
                ...
                deb.api.getAccountState(receipt.from).then(function (state) {
                    ...
                    return deb.api.getAccountState(coinbase).then(function (state) {
                        ...
                        return deb.api.getEventsByHash(receipt.hash);
                    });
                    ...
                    if (!testExpect.canExecuteTx) {
                        ...
                        deb.api.gasPrice().then(function (resp) {
                            expect(resp).to.have.property('gas_price');
...
function testVerifyAddress(testInput, testExpect, done) {
    ...
    deb.api.getAccountState(contractAddr).then(function(state){
        ...
        return deb.api.call(from.getAddressString(), contractAddr, testInput.value, parseInt(state.nonce) + 1, testInput.gasPrice, testInput.gasLimit, call);
    }).then(resp => {
...
function claimNas(contractType, done) {
    ...
    deb.api.getAccountState(sourceAccount.getAddressString()).then(function(resp){
