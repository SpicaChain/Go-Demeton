...
function prepareSource(done) {
    deb.api.getAccountState(originSource.getAddressString()).then(function (resp) {
        console.log("prepare source account state:" + JSON.stringify(resp));
        var nonce = parseInt(resp.nonce);

        source = Account.NewAccount();

        var tx = new Transaction(ChainID, originSource, source, deb.nasToBasic(1000), nonce + 1, "1000000", "200000");
        tx.signTransaction();
...
function prepareContractCall(testCase, done) {
    deb.api.getAccountState(source.getAddressString()).then(function (resp) {
...
function checkTransaction(txhash, done){
    ...
    var interval = setInterval(function () {
        ...
        deb.api.getTransactionReceipt(txhash).then(function (resp) {
            retry++;
...
function testCall(testInput, testExpect, done) {
    ...
    deb.api.call(from.getAddressString(), contractAddr, "0", 1, "1000000", "2000000", contract).then(function (resp) {
...
function testTransferFrom(testInput, testExpect, done) {
    ...
    allowanceOfNRC20(deploy.getAddressString(), from.getAddressString()).then(function (resp) {
        ...
        return deb.api.getAccountState(deploy.getAddressString());
    }).then(function (resp) {
        deployState = resp;
...
function approveNRC20(testInput, deployState, from, currentValue, done) {
    if (!Utils.isNull(testInput.approveValue)) {
        ...
        deb.api.sendRawTransaction(tx.toProtoString()).then(function (resp) {
...
function balanceOfNRC20(address) {
    ...
    return deb.api.call(address, contractAddr, "0", 1, "1000000", "200000", contract)
}
...
function allowanceOfNRC20(owner, spender) {
    ...
    return deb.api.call(owner, contractAddr, "0", 1, "1000000", "2000000", contract)
}
