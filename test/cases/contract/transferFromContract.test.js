...
describe('test transfer from contract', function () {
    ...
    it ('1. send 10 deb to contract address', function (done) {
        nonce = nonce + 1;
        ...
        deb.api.sendRawTransaction(tx.toProtoString()).then(function(resp) {
            console.log("----step3. send deb to contract address: ", resp);
            checkTransaction(resp.txhash, function(resp) {
                expect(resp).to.not.be.a('undefined');
    ...
    it ('2. send 5 deb from contract address to toAddress', function (done) {
        nonce = nonce + 1;
        var contract = {
        ...
        deb.api.sendRawTransaction(tx.toProtoString()).then(function (resp) {
            ...
            checkTransaction(resp.txhash, function(resp) {
                try {
                    ...
                    deb.api.getAccountState(toAddress.getAddressString()).then(function(resp) {
                        expect(resp.balance).equal("5000000000000000000");
                        return deb.api.getAccountState(contractAddress);
                    }).then(function(resp){
                        expect(resp.balance).equal("5000000000000000000");
                        return deb.api.getEventsByHash(hash);
                    }).then(function(resp){
    ...
    it ('3. send 5 deb to contract and try to send 11 deb from contract toAddress',
     function (done) {
        nonce = nonce + 1;
        var contract = {
            "function": "transferSpecialValue",
    ...
    it ('4. send 5 deb to contract and try to send 10 deb from contract toAddress',
    function (done) {
       nonce = nonce + 1;
       var contract = {
           "function": "transferSpecialValue",
