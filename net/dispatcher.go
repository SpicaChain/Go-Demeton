...
var (
	metricsDispatcherCached     = metrics.NewGauge("deb.net.dispatcher.cached")
	metricsDispatcherDuplicated = metrics.NewMeter("deb.net.dispatcher.duplicated")
)
...
func (dp *Dispatcher) PutMessage(msg Message) {
	// it's a optimize strategy for message dispatch, according to https://github.com/cyber-demeton/go-demeton/issues/50
	hash := msg.Hash()
...
func metricsDuplicatedMessage(messageName string) {
	...
	meter := metrics.NewMeter(fmt.Sprintf("deb.net.dispatcher.duplicated.%s", messageName))
	meter.Mark(int64(1))
}
