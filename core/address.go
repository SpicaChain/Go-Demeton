...
// Address design of demeton address
/*
[Account Address]
Similar to Bitcoin and Ethereum, Demeton also adopts elliptic curve algorithm as its basic encryption algorithm for Demeton accounts.
...
The final address of Demeton Wallet should be:  n1TV3sU6jyzR4rJ1D7jCAmtVGSntJagXZHC
...
For more information, plz check (https://github.com/cyber-demeton/wiki/blob/master/tutorials/%5BEnglish%5D%20Nebulas%20101%20-%2003%20Smart%20Contracts%20JavaScript.md) and [rpc.sendTransaction](https://github.com/cyber-demeton/wiki/blob/master/rpc.md#sendtransaction).
...
Addition of extended information allows the addition of another element verification to the Demeton Wallet APP. For example:
...
func AddressParse(s string) (*Address, error) {
	if len(s) != AddressBase58Length || s[0] != DemetonFaith {
