...
func NewBlockChain(deb Deblet) (*BlockChain, error) {
	if deb == nil || deb.Config() == nil || deb.Config().Chain == nil {
		return nil, ErrNilArgument
	}

	var gasPrice, gasLimit *util.Uint128
	var err error
	if 0 == len(deb.Config().Chain.GasPrice) {
		gasPrice = util.NewUint128()
	} else {
		gasPrice, err = util.NewUint128FromString(deb.Config().Chain.GasPrice)
		if err != nil {
			return nil, err
		}
	}

	if 0 == len(deb.Config().Chain.GasLimit) {
		gasLimit = util.NewUint128()
	} else {
		gasLimit, err = util.NewUint128FromString(deb.Config().Chain.GasLimit)
		if err != nil {
			return nil, err
		}
	}
	...
	blockPool.RegisterInNetwork(deb.NetService())

	txPool, err := NewTransactionPool(327680)
	if err != nil {
		return nil, err
	}
	txPool.setEventEmitter(deb.EventEmitter())
	if err := txPool.SetGasConfig(gasPrice, gasLimit); err != nil {
		return nil, err
	}
	txPool.RegisterInNetwork(deb.NetService())

	var bc = &BlockChain{
		chainID:            deb.Config().Chain.ChainId,
		genesis:            deb.Genesis(),
		bkPool:             blockPool,
		txPool:             txPool,
		storage:            deb.Storage(),
		eventEmitter:       deb.EventEmitter(),
		dvm:                deb.Dvm(),
		quitCh:             make(chan int, 1),
		superNode:          deb.Config().Chain.SuperNode,
		unsupportedKeyword: deb.Config().Chain.UnsupportedKeyword,
	}
...
func (bc *BlockChain) Setup(deb Deblet) error {
	bc.consensusHandler = deb.Consensus()

	if err := bc.CheckGenesisConfig(deb); err != nil {
		return err
	}
...
func (bc *BlockChain) CheckGenesisConfig(deb Deblet) error {
	genesis, err := DumpGenesis(bc)
	//db.genesis has and config lack
	if deb.Genesis() == nil && err == nil {
		deb.SetGenesis(genesis)
		if deb.Config().Chain.ChainId != deb.Genesis().Meta.ChainId {
			return ErrInvalidConfigChainID
		}
	} else if deb.Genesis() == nil && err != nil {
		...
		return CheckGenesisConfByDB(genesis, deb.Genesis())
	}

	logging.CLog().WithFields(logrus.Fields{
		"meta.chainid":           deb.Genesis().Meta.ChainId,
		"consensus.dpos.dynasty": deb.Genesis().Consensus.Dpos.Dynasty,
		"token.distribution":     deb.Genesis().TokenDistribution,
