...
func NewV8Engine(ctx *Context) *V8Engine {
	...
	if ctx.block.Height() >= core.DvmGasLimitWithoutTimeoutAtHeight {
...
func (e *V8Engine) RunContractScript(source, sourceType, function, args string) (string, error) {
	...
	if e.ctx.block.Height() >= core.DvmMemoryLimitWithoutInjectHeight {
		e.CollectTracingStats()
	...
	if e.ctx.block.Height() >= core.DvmGasLimitWithoutTimeoutAtHeight {
...
func getEngineByStorageHandler(handler uint64) (*V8Engine, Account) {
	...
	if engine.lcsHandler == handler {
		...
		// disable gcs according to issue https://github.com/cyber-demeton/go-demeton/issues/23.
		return nil, nil
