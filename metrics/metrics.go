...
// Deblet interface breaks cycle import dependency.
...
func Start(deb Deblet) {
	logging.VLog().Info("Starting Metrics...")

	go (func() {
		tags := make(map[string]string)
		metricsConfig := deb.Config().Stats.MetricsTags
		...
		tags[chainID] = fmt.Sprintf("%d", deb.Config().Chain.ChainId)
		go collectSystemMetrics()
		InfluxDBWithTags(metrics.DefaultRegistry, interval, deb.Config().Stats.Influxdb.Host, deb.Config().Stats.Influxdb.Db, deb.Config().Stats.Influxdb.User, deb.Config().Stats.Influxdb.Password, tags)
